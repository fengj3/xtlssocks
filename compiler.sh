...
compiler() {
  ...
  CGO_ENABLED=0 GOOS=$1 GOARCH=$2 go build -o bin/tlssocks-${1}-${2}${suffix} cmd/tlssocks/tlssocks.go
  CGO_ENABLED=0 GOOS=$1 GOARCH=$2 go build -o bin/tcpproxy-${1}-${2}${suffix} cmd/tcpproxy/tcpproxy.go
  CGO_ENABLED=0 GOOS=$1 GOARCH=$2 go build -trimpath -o bin/tlssocksproxy-${1}-${2}${suffix} cmd/tlssocksproxy/tlssocksproxy.go
